    ...
    *,
    ...
    for index, (window, label) in enumerate(zip(windows, labels, strict=True)):
        ...
        if not (
            ...
            and seen % config.update_stride == 0
        ):
            continue
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    config = replace(
        ...
        update_batch_size=(
            ...
            if args.update_batch_size is None
    ...
    if (
        ...
        or config.update_stride != 64
        or config.recent_samples != 320
    ):
        raise ValueError(
            "This replay requires the selected 64/64/320 NeuroOnline policy; "
            f"checkpoint has {config.history_threshold}/{config.update_stride}/"
            f"{config.recent_samples}."
        )
    ...
    expected_updates = len(
        [
            ...
            if seen >= config.history_threshold and seen % config.update_stride == 0
        ]
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--update-stride", type=int, default=64)
    parser.add_argument("--recent-samples", type=int, default=320)
