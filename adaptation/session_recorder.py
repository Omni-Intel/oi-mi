...
class SessionRecorder:
    ...
    def pull(self) -> np.ndarray:
        ...
        if values.size == eeg.shape[1] and np.all(np.isfinite(values)):
            self._timestamp_chunks.append(values.copy())
        else:
            self._timestamp_chunks.append(
                np.full(eeg.shape[1], np.nan, dtype=np.float64)
            )
        if (
    ...
    def add_event(
        ...
        **payload: Any,
        ...
        event = SessionEvent(
            ...
            relative_time_sec=event_time - self._started_at,
    ...
    def export(self, output_dir: Path, *, metadata: dict[str, Any]) -> Path:
        ...
        self._save_npy_atomic(
            output_dir / "continuous_sample_timestamps.npy",
            self.to_timestamps(),
        )
    ...
    def to_timestamps(self) -> np.ndarray:
        """Return one source timestamp per exported continuous EEG sample."""

        if not self._timestamp_chunks:
            return np.empty((0,), dtype=np.float64)
        return np.concatenate(self._timestamp_chunks).astype(np.float64)
